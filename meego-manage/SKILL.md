...
| 参数 | 说明 |
|---|---|
| `work_item_type_key` | 工作项类型；不传默认 `story`，查迭代用 `sprint` |
| `work_item_name` | 名称搜索；兼容 `query` / `keyword` / `name` / `sprint_name` / `iteration_name` 别名 |
| `only_mine` | 是否只查当前用户相关工作项；`story/issue/task` 默认 `true`，`sprint` 默认 `false` |
| `businesses` | 单项目查询的业务线过滤，传业务线 `id` 列表；可先用 `action=project` 获取 `businesses` |
| `page_size` / `page_num` | 分页参数；查迭代建议 `page_size=100` |

查项目迭代 / Sprint 时，必须按项目全量口径确认，不要只依赖默认“我相关”的结果。示例：

```json
{
  "action": "query",
  "project_key": "ttarch",
  "work_item_type_key": "sprint",
  "work_item_name": "GDPA",
  "only_mine": false,
  "page_size": 100
}
...
如果 Meego 页面使用了业务线过滤（如 VC-Server Arch），先 `action=project` 查询该空间业务线，拿到对应业务线 `id` 后在 `businesses` 里传入；否则可能漏掉页面中可见的 GDPA 迭代。
