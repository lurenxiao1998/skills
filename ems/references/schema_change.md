...
> 🛑 **Action renamed**: The previous `create_schema_change` action has been **renamed to `change_rds_table`** (Path 2 in the decision tree). The input shape is unchanged — `rds_uri` + `tables[]` still works verbatim. Callers using the old name will fail with `unknown action`; replace the action string and re-run. The legacy alias is **NOT** kept on purpose so stale fixtures fail loudly rather than silently.
>
> Five "entity / table change" paths now share this lifecycle:
>
> | Path | Action | When to use | Doc |
> |---|---|---|---|
> | 1 | `execute_rds_ddl` | RDS OpenAPI direct (no EMS entity update) | `rds_deployment.md` |
> | 2 | `change_rds_table` | Most common — DDL + auto-infer entity | this file |
> | 3 | `change_rds_table_with_entity_edits` | DDL + user-edited entity fields (description / PII / tags) | this file |
> | 4 | `update_rds_entity` | RDS entity-only metadata change (no DDL, no IaC). Renamed from `update_entity`; abase/redis/etc. will be covered by future `update_<storage>_entity` actions. | `entities.md` |
> | 5 | `import_online_table` | Register an already-existing physical table as an EMS entity | `entities.md` |
>
> RDS / MySQL only across all five paths. `format_version=V1` entities are read-only and rejected by every write path. The decision tree lives in `SKILL.md`.

This file documents the full RDS-table CREATE / ALTER lifecycle for Paths 2 and 3 — five tightly-coupled actions:

| action | role |
|---|---|
| `validate_ddl_change` | (read-only) validate DDL + infer the resulting `entity_mapping` |
| `change_rds_table` | (write, Path 2) publish a workflow on top of validated DDL with auto-inferred entity |
| `change_rds_table_with_entity_edits` | (write, Path 3) `change_rds_table` + per-row `entity_overrides` + `precheck_entity` |
...
2. `change_rds_table` (write) — publish a workflow on top of the validated DDL. Returns a `workflow_id` + `workflow_url`.
...
Both `validate_ddl_change` and `change_rds_table` operate on a **batch** of tables on the same RDS via a required `tables: []` array. CREATE and ALTER rows can be mixed in the same batch; the skill auto-detects each row's operation by probing EMS, runs `ValidateAndInferFromDdl` concurrently per row (limit 5), and (for `change_rds_table`) publishes the entire batch as **a single workflow** via one `CreateSchemaChangeV2` call.

`change_rds_table` **internally re-runs `validate_ddl_change`** with the same parameters before publishing — there is no caching layer, so the inferred `entity_mapping` always matches what the backend will receive.

## Batch input contract (`tables: []`)

Both `validate_ddl_change` and `change_rds_table` take a **required `tables: []` array** — one entry per table you want to change on the same RDS. A single-table change is just a `tables: [{...}]` of length 1. Mixing CREATE and ALTER rows in the same batch is allowed; the skill auto-detects each row's operation by probing EMS for the entity. Validation is fanned out concurrently (limit 5) and the publish goes out as a **single** workflow regardless of row count.
...
| `skip_validate_single_rds_ddl_sql` | bool | (default `false`) Asks EMS to skip the per-vregion RDS-OpenAPI pre-validation hop. Use this **only** as an escape hatch when the upstream RDS OpenAPI is flaky. The skill appends an explicit entry to `risk_warnings` in both preview and publish payloads because the RDS platform still re-validates the SQL when the workflow is published — any genuinely invalid DDL just shifts its failure later in the flow. Applies to `validate_ddl_change` / `change_rds_table` / `change_rds_table_with_entity_edits`. |
| `confirm` | bool | Write actions (`change_rds_table`, `change_rds_table_with_entity_edits`) only — preview vs. publish. |
...
| `is_sharding_table` | bool | No (default `false`) | **CREATE rows only.** `change_rds_table` only. |
| `sharding_key` | string | When `is_sharding_table=true` | `change_rds_table` only. |
...
| `entity_overrides` | object | No (Path 3 only) | `change_rds_table_with_entity_edits` only. `{fields: [{name, description?, tags?, pii_labels?, ...}]}`. Each entry patches the inferred entity field with matching `name` (case-insensitive). Unrecognised fields surface via `precheck_entity`. |
...
3. Backend-inferred fallback — see the next section for the full vregion inference order
...
### Auto-inferred `vregions` (ALTER) — uses live RDS deployment, not entity metadata

When neither `tables[i].vregions` nor the shared `vregions` is set, the resolver picks the default by walking this list and returning the first non-empty result:

1. **ALTER, live deployment probe** (preferred): `GetEntityOnlineDdl(storage_uri, table_name)` is called once per row; only the vregions where the physical RDS table is actually deployed (`status == 1` with a non-empty `CREATE TABLE ...` snapshot) are kept. Vregions present in the entity catalog but missing from the live deployment (`status == 2` / empty SQL) are reported under `tables[i].notes` as `"vregions inferred from online RDS deployment (N deployed); skipped M not-deployed: [...]"`.
2. **ALTER, entity metadata fallback**: when the live probe fails (transport error, 502) or every region in the response is non-deployed, the resolver falls back to the existing entity's `idcs` / `vdcs` and notes `"vregions inferred from existing entity (N idcs)"`. A separate `"... probe failed ..."` note is appended when step 1 errored, so users can see why the default shifted.
3. **CREATE, parent storage**: for new entities the live deployment is meaningless (no table yet) — the probe is skipped entirely and the parent storage's `idcs` / `vdcs` is used (`"vregions inferred from storage \"...\" (N idcs)"`).

The probe-first behaviour exists because the EMS entity catalog can claim coverage for vregions that the physical RDS never deployed; running the validator against an absent database fails with `database does not exists` and used to manifest as opaque `inferred_entity_mapping: null` errors. Callers can still pin any vregion list explicitly — both top-level `vregions` and per-row `tables[i].vregions` short-circuit the probe and are honoured verbatim.

## `validate_ddl_change`

Validate a batch of DDL SQL statements against EMS and return per-row diagnostics + the inferred `entity_mapping` for each table. **Read-only** — does not publish anything. Use it to sanity-check a batch before invoking `change_rds_table`.
...
The preview payload uses a **fixed schema** across `validate_ddl_change`, `change_rds_table`, `change_rds_table_with_entity_edits`, and `update_rds_entity`. Every listed key is **always present** (with an empty list / map when there is nothing to surface) so the agent UI never has to do `if key exists` dancing — see `preview_report.go` for the canonical contract.

- `storage_uri`, `psm`, `shared_vregions` — the shared resolution echoed back.
- `tables: [{entity_uri, table_name, operation, table_operation, entity_operation, vregions, regional_sql_list, validation, field_diff, entity_schema, inferred_entity_mapping, notes?, ...}]` — one entry per `tables[i]`.
  - `operation` is the user-facing operation label as an enum: **`CREATE` / `ALTER`** (uppercase) — derived from the DDL keyword for DDL paths, `UPDATE_RDS_ENTITY` for `update_rds_entity`. When in doubt, show this value.
  - `table_operation` (`create_table` / `alter_table`) is the legacy lower-case form retained for programmatic consumers (BPM, downstream tooling). The user-facing label is always `operation`.
  - `entity_operation` (`create` / `update`) is what EMS does to the entity catalog. Only surface this to the user when it diverges from `table_operation`.
  - `regional_sql_list: [{vregion, sql}, ...]` — per-vregion DDL exactly as the workflow will receive it. Always present (empty for entity-only `update_rds_entity`).
  - `validation: {errors: [...], warnings: [...], regional: [...]}` — see below for the fixed schema.
  - `field_diff: {added, removed, modified, unchanged_count}` — see below.
  - `entity_schema: {fields: [{entity_field, entity_type, table_column, column_type, description}, ...], primary_key, indices}` — the user-facing **post-change** schema view. Show this as a table.
- `summary: {total, create_count, alter_count, vregion_count, errors_count, warnings_count, all_valid}` — always present, all numeric. `all_valid` is `errors_count == 0`. `create_count` / `alter_count` reflect the **table-level** operation (DDL keyword), so a `CREATE TABLE` row whose entity is already registered still counts as a CREATE — matching what the user submitted.
- `risk_warnings: [string, ...]` — batch-wide advisory strings (e.g. `skip_validate_single_rds_ddl_sql=true`). Always present, empty when there is nothing to flag.
- `next_steps: [{id, label, hint}, ...]` — present on every preview. Always contains at least `{id: publish}` (re-run with `confirm=true`); `change_rds_table` / `change_rds_table_with_entity_edits` / `update_rds_entity` also include `{id: edit_entity_fields}` so users know they can drop into the override flow if they want to tweak codecs / inner fields / descriptions before publishing.

#### `validation` block (fixed schema)

`validation.errors[]` carries **blocking** issues — when this list is non-empty for any row, the publish flow short-circuits and the dispatcher returns the preview alongside an `ApiBizError` so the user can see every failure in one shot. `validation.warnings[]` carries non-blocking advisories. Each issue is a struct:

```yaml
{
  level:    "ERROR" | "WARNING",
  source:   "ddl_regional" | "heuristic" | "consistency" | "precheck" | "entity_rules" | "transport",
  code:     "<heuristic rule name, if any>",
  vregion:  "<region tag, if any>",
  message:  "<human-readable text>"
}
```

The `regional[]` array is passed through verbatim from EMS' `regional_ddl_validation_data_list` for programmatic consumers; the user-facing entries are in `errors` / `warnings`.

Heuristic-rule contract — every `heuristic_rules[i]` entry from EMS is mapped to one of two buckets based on its `can_skip_type` enum:

- `can_skip_type=1` (Skippable) → **Warning** (`source: "heuristic"`). The user MAY proceed without fixing; the agent should still display the rule so the choice is conscious.
- `can_skip_type=2` (Forbidden) → **Error** (`source: "heuristic"`). EMS itself enforces this: when any rule is Forbidden, EMS returns no `inferred_entity_mapping`, so `change_rds_table` cannot proceed until every Forbidden rule is resolved.
- Older / alternate EMS versions emit a boolean `can_skip` instead. The parser still respects it: `can_skip=true` ↔ Skippable, `can_skip=false` ↔ Forbidden. Unknown enum values default to Forbidden so a future EMS-side enum addition cannot silently bypass the publish flow.

#### `field_diff` block (fixed schema)

Computed against the **live online entity** (`rctx.DraftMapping` for ALTER, the current `mapping` for `update_rds_entity`). For CREATE the live entity is nil → diff degenerates to "all added".

```yaml
field_diff:
  added:    [{name, type, codec?, description?, innerFields?, tags?, pii_labels?, ...}, ...]
  removed:  [{name, type, codec?, description?, innerFields?, ...}, ...]
  modified:
    - name: "<field name>"
      changes: [{key, old, new}, ...]   # per-top-level-key diff (excludes name + innerFields)
      inner_field_diff:                  # recursive — only present when innerFields differ
        added: [...]
        removed: [...]
        modified: [...]
        unchanged_count: N
...
The recursive `inner_field_diff` is the key piece for JSON / struct columns — when a JSON field's `innerFields` change, the diff drills down so the user sees **which** sub-field was added / removed / modified rather than two giant innerFields blobs side by side. The recursion is unbounded depth.

> ⚠️ **Heuristic-rule contract** — every `heuristic_rules[i]` entry is mapped to one of two buckets based on its `can_skip_type` enum:
>
> - `can_skip_type=1` (Skippable) → **Warning**. Surfaced in `warnings`. The user MAY proceed to `change_rds_table` without fixing it; the agent should still display the rule so the user can decide consciously.
> - `can_skip_type=2` (Forbidden) → **Error**. Surfaced in `validation.errors`. The user MUST modify the DDL to clear the rule. EMS itself enforces this: when any rule is Forbidden, EMS returns no `inferred_entity_mapping`, so `change_rds_table` cannot proceed until every Forbidden rule is resolved.
...
`confirm` omitted / `false` returns the **fixed-schema** preview payload defined in `Response highlights` above — same shape across `validate_ddl_change` / `change_rds_table` / `change_rds_table_with_entity_edits` / `update_rds_entity`. Key fields the agent UI should surface to the user:

- `display_action` — `"Release RDS Table"` for a single-row batch, `"Release RDS Tables"` (plural) for ≥ 2 rows. Don't show `change_rds_table`.
- `action: "change_rds_table"` — the dispatcher name for programmatic consumers.
- `summary: {total, create_count, alter_count, vregion_count, errors_count, warnings_count, all_valid}` — render this as the "header" of the preview card.
- `preview: true`, `confirm_required: true`, `published: false` — gating flags.
- `risk_warnings: []` — batch-wide advisories (e.g. `skip_validate_single_rds_ddl_sql`).
- `tables[]` per-row: `operation` (CREATE/ALTER), `vregions`, `regional_sql_list`, `field_diff`, `entity_schema`, `validation`, sharding params, etc. Render each as a per-table card with:
  - "Per-VRegion SQL" — derived from `regional_sql_list`.
  - "Field changes" — derived from `field_diff` (added / removed / modified). Show `field_diff.modified[*].inner_field_diff` when present so users see exactly which JSON sub-field changed.
  - "Field ↔ Column mapping" — derived from `entity_schema.fields[]`.
  - "Validation" — `validation.errors[]` (Error level) + `validation.warnings[]` (Warning level). Always show even when empty, so users learn that "no errors / no warnings" is a confirmed state and not a missing render.
- `next_steps: [{id: "publish", ...}, {id: "edit_entity_fields", ...}]` — always emit both entries (the publish step plus a path-3 "edit overrides" hint), regardless of whether the user actually intends to use them. The agent should render them as actionable links / buttons. When EVERY row is judged "entity-neutral" (e.g. only `ENGINE = X` / `AUTO_INCREMENT = N`-class table options), an additional `{id: "use_path1", ...}` entry is appended pointing at `execute_rds_ddl` (Path 1) so users have the option to skip the EMS workflow + Owner approval entirely. See `rds_deployment.md` → "When is Path 1 safe? — entity-neutral ALTER 速查" for the full whitelist / blacklist.
...
> **`field_diff` for ALTER is computed against the live online entity** — not against the inferred entity. EMS persists the inferred entity record at workflow-create time but the RDS DDL only deploys at the BPM stage; using the live state as the diff baseline means a re-submitted workflow (post-cancel) still shows the user "this is the actual net change I'm asking for". If the diff comes back empty for an ALTER, the entity-side change has already been recorded by a prior workflow run and the workflow is only needed to drive the RDS deploy; `regional_sql_list` always reflects the SQL that will be re-run.
...
- `owner_gate_preview` — the same payload `approve_workflow_owner_gate` returns in preview mode (sans the duplicated `workflow_id` / `workflow_url` / `display_action`), plus a `next_action: "approve_workflow_owner_gate"` field. Keys to surface to the user:
  ...
  - `next_steps` — array of short Chinese guidance strings phrased for the publish flow, e.g. `["当前用户 X 在 Reviewer 列表中，确认后即可完成 Owner 审批"]`. CLI commands and field names (`confirm=true`, `action=...`) are deliberately suppressed so the user sees a clean prompt; the agent already knows the next action via `next_action`.
- The parent `next_steps` array is intentionally short (`["工作流已发布，请确认下方 Owner 审批信息后继续"]`) — the embedded gate carries the actionable detail.
- If the freshly-created workflow is not yet queryable (rare transient race) or the publisher is not in the reviewer set, `owner_gate_preview.unavailable: "<reason>"` carries the error and the parent + embed `next_steps` fall back to a single-entry array pointing at `<workflow_url>` so the user can drive the gate manually in the EMS UI. The publish itself is **never masked** by this embed step.

The verbose EMS envelope (`change_rds_table_code` / `change_rds_table_message`) is **not** surfaced in the user-facing fields anymore; if a programmatic consumer needs it, read the raw `result.data.data` envelope instead.
...
- `confirm_required: true`, `next_steps: [string]` (re-run with `confirm=true`).

### Confirm output (`confirm=true`)

- `display_action`, `published: true`, `workflow_id`, `workflow_url`, `next_steps: [string]`.
...
- `display_action: "Close RDS Workflow"`, `published: true`, `workflow_id`, `workflow_url`, `next_steps: [string]`.
...
- **Validate** → "Here's how the change looks across regions, and the resulting table schema. Approve to publish?" Drive the table-by-table view from `tables[i].entity_schema` — never dump the raw `inferred_entity_mapping`. For multi-row batches, render one card per `tables[i]` (Entity URI / Table Name / DDL / fields). Render `entity_schema.fields[]` as a markdown table (Entity Field / Type / Table Column / Column Type / Description) regardless of whether the row is CREATE or ALTER — show the full post-change schema, do not synthesize a diff. Lead the overall response with the `summary` counts (`N tables: X CREATE / Y ALTER`, plus `warnings_count` when set). Use `tables[i].operation` (= the table-level operation, derived from the DDL keyword) as the per-row label — **never** describe a `CREATE TABLE` row as "ALTER" just because the EMS catalog already has the entity. The user wrote `CREATE TABLE`; the table is being CREATEd. See the divergence guidance below.
  - **Errors (blocking)** — when `validation.ok=false` for any row, lead with the `validation.errors` list and **refuse to suggest `change_rds_table`** until the user fixes the DDL. Forbidden heuristic rules (`can_skip_type=2`) land in this bucket and MUST be resolved before publishing — EMS itself withholds the inferred mapping for them, so the publish path cannot proceed. Phrase as "❌ 这条 DDL 还没法发布，请先修正以下问题"; do not propose `confirm=true`.
