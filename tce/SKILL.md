---
name: tce
description: Query TCE service deployment state, cluster details, and Pod information. Use whenever the user mentions TCE, wants to check service or deployment state, view Pod information or Pod list, check cluster details or instances, or troubleshoot TCE service state issues. Also trigger before auxiliary skills when the user asks whether a service exists, is running, has Pods, or needs deployment-state evidence; this skill collects state evidence and does not execute deployments.
---
...
如果 CLI 返回异步 `running` / `session_id` 状态，只轮询同一 session 最多 2 次；必须拿到与本次 action 匹配的响应后再进入下一步，仍未完成时输出结构化 unresolved 状态。
...
本 skill 只查询 TCE 服务、集群、Pod 和部署状态证据，不创建、发布、回滚或修改部署。用户要求执行部署时，先用本 skill 收集必要状态证据，再交给支持部署执行的独立 skill。
...
最终回答前逐项核对用户请求中的服务、VRegion、Pod、集群和部署状态维度；每个被请求的维度必须引用已返回的 TCE 证据，或明确标为 `unavailable` 并说明缺少哪一步证据。

服务状态、部署状态、Pod 状态或 “no service / not running” 类问题必须先走 `list_services` 或已知 `service_id` 的 `get_service_detail`；只有用户明确禁止 TCE 或 TCE 证据完成后，才切到仓库搜索、配置推断或辅助 skill。

任何 fallback 都必须保持原始 TCE 证据目标（服务 / VRegion / Pod / 集群 / 部署状态）；不要把服务状态答案改写成代码库搜索、shell 探测或浏览器调查。

进入 `get_service_detail`、`list_service_clusters` 或 Pod 查询前，先复述并锁定 `list_services` 命中的服务、选定的 `service_id`、`vregion` 和当前子目标；后续 action 不得隐式切换这些范围。
...
当用户意图切换区域或服务目标时，必须重新绑定 `service_id`；进入 Pod / 集群查询前还必须重新绑定该 VRegion 下的 `cluster_id`。
...
当 TCE 调用失败或只返回部分结果时，最终回答必须保持 unresolved 状态，点名缺失的服务 / VRegion / Pod / 集群 / 部署状态维度；不要用配置、代码库或经验判断补全没有证据的结论。

失败恢复顺序固定为：① 用修正后的参数重试同一 TCE action 一次；② 使用文档中支持的相邻 TCE action 补证据；③ 仍失败则停止并输出缺失维度。完成这三步前，不切换到 repo / shell / browser 路径。
