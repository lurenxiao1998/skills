...
| skill wrapper / session / transport failure before API result | CLI wrapper 没把请求送到 DECC，尚无平台事实 | 读操作可用同一 JSON 直接执行 `gdpa-cli run decc-desrpc --input ...` 一次；写/提交操作遇到 `network` / `api` / `runtime` 失败时先核对 DECC 当前资源状态或 preview，并让用户明确确认是否重放；仍失败就报告 `failure_class` / `retry_hint`，不要转向 repo search |
