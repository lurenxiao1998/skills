---
name: decc-desrpc
description: Query and manage DECC DES-RPC caller/callee channels, data rules, data versions, and field-level DES annotations for cross-region RPC routing. Use when the user mentions DECC, DES-RPC, caller/callee channel config, channel-rule checks, caller-to-callee method routing, cross-region RPC routing, data-rule version history, ACL/permission blockers, submit/audit flows, or RPC field 打标.
---
...
- 回答"某 caller 是否能调某 callee/method"、"channel 下有哪些 data rule/version"、"为什么被 ACL/权限挡住"这类隐式 channel-rule 问题
...
Read-only lookup matrix:

| User wording | Use this action | Minimum final evidence |
|---|---|---|
| "which channel / caller / callee / VGeo" | `list_channels` | channel name, VGeo/site, owners or state when present |
| "methods / data rules under channel" | `list_data` | exact `data_id`, method/path, latest version state |
| "version history / reason / direction pairs" | `list_data_versions` | version, states, description/reason, direction_pairs |
| "caller can reach callee / method route" | `query_caller_channel` | caller, callee, method filter, enabled/state |
| "catalog code / rejection wording" | `fetch_meta` | catalog label/code or matching rejection template |
| "IDL exists / schema source" | `load_idl` | IDL payload or explicit BAM blocker |
...
**On-axis fallback ladder.** When a read action fails, stay inside `decc-desrpc`:
1. Retry the same action once with corrected identifiers (`data_id` over ambiguous `channel` + `data_name`, explicit `vregion` / site / method when known).
2. Try the sibling DECC read that resolves the missing key (`list_channels` → `list_data` → `list_data_versions`, or `query_caller_channel` for caller/callee routing).
3. Stop and report the attempted action, failure class, and next DECC input needed from the user; do not switch to repo search, generic browsing, or unrelated tools for DECC platform facts.

If the skill wrapper itself fails before the API call (transport/session/argument wrapper error), run the same JSON once through the direct CLI command above before any other recovery path. Treat the direct CLI result as the authoritative DECC failure or success. For write/submit actions, do not automatically replay after `network` / `api` / `runtime` failures; first verify the current DECC resource state or preview and ask the user for explicit confirmation.

<details>
<summary>Write-only DES tagging guidance; read-only lookup tasks can skip to Actions.</summary>
...
| User ask / missing evidence | First action | Bounded fallback |
|---|---|---|
| Find a caller/callee channel or VGeo route | `list_channels` with `name` / `vgeo` | If names collide, refine `name` or ask for channel owner / PSM; then `query_caller_channel` only when caller-callee routing is requested. |
| List RPC methods / HTTP paths under a channel | `list_data` with exact `channel` | If the channel is unknown, run `list_channels`; after auth/runtime errors, report the blocker instead of repo search. |
| Inspect version history, state, reason, direction pairs | `list_data_versions` with `data_id` when known | If `channel` + `data_name` is ambiguous or fails, re-query by `data_id`; if the user lacks permission, report the auth blocker and needed owner. |
| Check caller → callee method routing | `query_caller_channel` with `callee`, plus `caller` / `method` / sites when known | If it returns empty, relax one filter at a time inside the same action before concluding no configured route was found. |

For read-only tasks, finish from the DECC action output plus the output schema. Only use repository files for IDL/tagging context after a DECC read proves the data rule or channel that needs code context.
...
Final answers for duplicate-name cases must include the selected `data_id`, version number, state, and the reason/direction pair fields when enriched output is available.

Duplicate-name rule: when `channel` + `data_name` resolves to multiple or no versions, first run `list_data` to copy the exact `data_id`, then rerun `list_data_versions` with that `data_id`. Auth or runtime failures on this endpoint are reported as DECC access blockers; they are not a reason to search the repository for guessed versions.
...
Before answering, inspect the packaged result after validation/API execution:
- `evidence_ready=true` means the DECC action returned platform data; answer from `data` and cite the exact `action` / identifiers used.
- `evidence_ready=false` means retrieval failed or produced no DECC evidence; do not infer from local IDL, repository search, or memory. State the `attempted_action`, `error_detail.failure_class`, and `error_detail.retry_hint`, then ask for the missing identifier or access fix.
- Validation failures raised before `call_api` may not include `attempted_action` / `evidence_ready`; fix the reported input error instead of treating it as a packaged retrieval result.
- For preview/codegen/retrieval blockers, use the blocked-result shapes in [references/output-schemas.md](references/output-schemas.md) rather than presenting a successful channel/data-rule answer.
