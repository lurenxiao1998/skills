...
6. After a Metrics query failure, keep the first two recovery attempts inside `metrics` / `gdpa-cli run metrics`; repo search, file search, or web browsing are not observability evidence for Metrics answers.
...
If the command fails, retry on-axis only: first correct the same Metrics request (mode, `psm`/`metric`, `vregion`, time range, filters), then retry the equivalent raw `gdpa-cli run metrics` payload. Escalate to the user with the exact `query_error` when both attempts fail instead of switching to repo or browser investigation.
...
Before finalizing, verify the answer covers every requested criteria dimension and names the evidence used. Always include the queried time range with timezone, `vregion` / `_region`, aggregation semantics (aggregator, rate/downsample/top-k/filter grouping when relevant), and whether the result is no-data, partial-data, or failed.
