...
```json
{
  ...
  "data": {
    ...
    "psm": "your.service.psm",
    "vregion": "Singapore-Central",
    "_region": "Singapore-Central",
    "time_range": "2026-05-18T01:00:00+08:00 ~ 2026-05-18T02:00:00+08:00",
    "section_count": 3,
    "sections": [
      {
...
| `authentication failed` | Missing or wrong JWT for the Metrics control plane | Re-authenticate for the requested `vregion` / `_region`, then retry the same Metrics payload |
| `series_count: 0` | No traffic, wrong time window, or wrong region/filter | Keep the same mode and adjust only the time window, `vregion`, or filters the user allowed |
| Backend/API failure | Metrics gateway or transport issue | Retry the same payload once; if it still fails, report `query_error` and ask for auth/region confirmation |

Recovery stays in the Metrics command family. Do not use repo search, file search, or web browsing as fallback evidence for a monitoring-data question.
